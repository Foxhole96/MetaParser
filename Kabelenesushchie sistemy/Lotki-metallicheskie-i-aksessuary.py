...
url = 'https://001.com.ua/lotki-metallicheskie-i-aksessuary'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            height = test_pars_capitalize(soup=soup, name='Тип')
            width = test_pars_capitalize(soup=soup, name='Тип')
            lenth_mm = test_pars_capitalize(soup=soup, name='Тип')

            try:
                material = soup.find('div', id='chars').find('ul').find('li').text.strip()
                if material and 'Материал' in material:
                    material = material.split(':')[1].strip().capitalize()
                else:
                    material = ''
            ...
            data.append({
                ...
                'Длина': f'Русский ~ {lenth_mm}',
                'Материал': f'Русский ~ {material}',
            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('lotki-metallicheskie-i-aksessuary.csv', 'w', newline='', encoding='utf-8-sig') as f:
