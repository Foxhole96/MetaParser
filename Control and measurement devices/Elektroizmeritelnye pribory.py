...
url = 'https://001.com.ua/elektroizmeritelnye-pribory'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            device_type = test_pars_capitalize(soup=soup, name='Тип изделия')
            connect = test_pars_capitalize(soup=soup, name='Подключение')
            min_value = test_pars(soup=soup, name='Минимальное значение')
            max_value = test_pars(soup=soup, name='Максимальное значение')
            current_type = test_pars_capitalize(soup=soup, name='Род тока')
            num_of_phases = test_pars(soup=soup, name='Количество фаз')
            indicator_type = test_pars_capitalize(soup=soup, name='Тип индикатора')
            montazh = test_pars(soup=soup, name='Монтаж')
            montazh = montazh.strip().replace('в кнопочный пост',
                                              'В кнопочный пост').replace('на DIN-рейку',
                                                                          'DIN-рейка').replace('на панель', 'На панель')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Электроизмерительные приборы',
                ...
                'Минимальное значение': f'Русский ~ {min_value}',
                'Максимальное значение': f'Русский ~ {max_value}',
                'Род тока': f'Русский ~ {current_type}',
                'Количество фаз': f'Русский ~ {num_of_phases}',
                'Тип индикатора': f'Русский ~ {indicator_type}',
        ...
        with open('elektroizmeritelnye-pribory.csv', 'w', newline='', encoding='utf-8-sig') as f:
