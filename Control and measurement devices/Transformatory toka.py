...
url = 'https://001.com.ua/transformatory-toka'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            rated_primary_current = test_pars(soup=soup, name='Номинальный первичный ток, А')
            accuracy_class = test_pars(soup=soup, name='Класс точности')
            tire_availability = test_pars(soup=soup, name='Наличие шины')
            calibration_interval = test_pars(soup=soup, name='Межповерочный интервал, лет')
            calibration_interval = f'{calibration_interval} лет'
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Трансформаторы тока',
                ...
                'Номинальный первичный ток': f'Русский ~ {rated_primary_current}',
                'Класс точности': f'Русский ~ {accuracy_class}',
                'Наличие шины': f'Русский ~ {tire_availability}',
                'Межповерочный интервал': f'Русский ~ {calibration_interval}',

            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('transformatory-toka.csv', 'w', newline='', encoding='utf-8-sig') as f:
