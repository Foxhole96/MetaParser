...
url = 'https://001.com.ua/schetchiki-elektroenergii'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            count_of_phases = test_pars(soup=soup, name='Количество фаз')
            num_of_tariffs = test_pars_capitalize(soup=soup, name='Количество тарифов')
            measured_energy = test_pars_capitalize(soup=soup, name='Измеряемая энергия')
            max_voltage = test_pars(soup=soup, name='Максимальный ток, А')
            connect = test_pars_capitalize(soup=soup, name='Подключение (по току)')
            interface = test_pars(soup=soup, name='Интерфейс передачи данных')
            dopolnitelno = test_pars_capitalize(soup=soup, name='Дополнительно')
            montazh = test_pars(soup=soup, name='Установка')
            if 'панель' in montazh:
                montazh = montazh.strip().replace('панель', 'На панель')
            accuracy_class = test_pars(soup=soup, name='Класс точности')

            print(availability, sku, montazh)
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Счетчики электроэнергии',
                ...
                'Количество фаз': f'Русский ~ {count_of_phases}',
                'Количество тарифов': f'Русский ~ {num_of_tariffs}',
                'Измеряемая энергия': f'Русский ~ {measured_energy}',
                'Максимальный ток': f'Русский ~ {max_voltage}',
                'Класс точности': f'Русский ~ {accuracy_class}',
                'Тип подключения': f'Русский ~ {connect}',
                'Способ монтажа': f'Русский ~ {montazh}',
                'Интерфейсы': f'Русский ~ {interface}',
                'Дополнительно': f'Русский ~ {dopolnitelno}',
            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('schetchiki-elektroenergii.csv', 'w', newline='', encoding='utf-8-sig') as f:
