...
url = 'https://001.com.ua/bytovye-istochniki-pitaniya-batareyki-akkumulyatory'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            size = test_pars(soup=soup, name='Типоразмер')
            nominal_battery_capacity = test_pars(soup=soup, name='Номинальная ёмкость аккумулятора, мА·ч')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Батарейки и аккумуляторы',
                ...
                'Типоразмер': f'Русский ~ {size}',
                'Номинальная ёмкость аккумулятора': f'Русский ~ {nominal_battery_capacity}',

            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('batareyki-akkumulyatory.csv', 'w', newline='', encoding='utf-8-sig') as f:
