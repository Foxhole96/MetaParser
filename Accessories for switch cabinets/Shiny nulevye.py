...
url = 'https://001.com.ua/shiny-nulevye'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            gabarit = test_pars(soup=soup, name='Габарит шины')
            gabarit = gabarit.replace(' мм', '')
            print(gabarit)
            num_of_holes = test_pars(soup=soup, name='Количество отверстий')
            color = test_pars_capitalize(soup=soup, name='Цвет изолятора')
            color = color.replace('-', '')
        ...
        with open('shiny-nulevye.csv', 'w', newline='', encoding='utf-8-sig') as f:
