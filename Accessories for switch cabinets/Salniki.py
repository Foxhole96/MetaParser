...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            device_type = test_pars(soup=soup, name='Тип сальника')
            device_type = f'Сальник {device_type}'
            cable_diameter = test_pars(soup=soup, name='Диаметр кабеля, мм')
            hole_diameter = test_pars(soup=soup, name='Диаметр отверстия бокса, мм')
            ...
            data.append({
                ...
                'Диаметр кабеля': f'Русский ~ {cable_diameter}',
                'Диаметр отверстия бокса': f'Русский ~ {hole_diameter}',
        ...
        with open('salniki.csv', 'w', newline='', encoding='utf-8-sig') as f:
