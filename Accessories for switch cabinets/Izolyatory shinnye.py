...
url = 'https://001.com.ua/izolyatory-shinnye'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            breakdown_voltage = test_pars(soup=soup, name='Напряжение пробоя, кВ')
            bolts_included = test_pars_capitalize(soup=soup, name='Болты в комплекте')
            bolt_type = test_pars(soup=soup, name='Тип болта')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Изоляторы шинные',
                ...
                'Напряжение пробоя': f'Русский ~ {breakdown_voltage}',
                'Болты в комплекте': f'Русский ~ {bolts_included}',
                'Тип болта': f'Русский ~ {bolt_type}',

            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('izolyatory-shinnye.csv', 'w', newline='', encoding='utf-8-sig') as f:
