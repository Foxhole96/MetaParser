...
url = 'https://001.com.ua/kontrolno-izmeritelnyy-instrument'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            measurement_method = test_pars_capitalize(soup=soup, name='Способ измерения')
            measured_parameters = test_pars_capitalize(soup=soup, name='Измеряемые параметры')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Контрольно-измерительный инструмент',
                ...
                'Способ измерения': f'Русский ~ {measurement_method}',
                'Измеряемые параметры': f'Русский ~ {measured_parameters}',

            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('kontrolno-izmeritelnyy-instrument.csv', 'w', newline='', encoding='utf-8-sig') as f:
