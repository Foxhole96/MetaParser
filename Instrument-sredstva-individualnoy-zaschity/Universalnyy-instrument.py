...
url = 'https://001.com.ua/universalnyy-instrument'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            crimping_tips = test_pars_capitalize(soup=soup, name='Опрессовка наконечников, мм²')
            stripping = test_pars_capitalize(soup=soup, name='Снятие изоляции, мм²')
            wire_cutting = test_pars_capitalize(soup=soup, name='Резка провода до ∅, мм')
            steel_wire_cutting = test_pars_capitalize(soup=soup, name='Резка стальной проволоки до ∅, мм')
            screw_cutting = test_pars_capitalize(soup=soup, name='Резка винтов, М')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Универсальный инструмент',
                ...
                'Опрессовка наконечников': f'Русский ~ {crimping_tips}',
                'Снятие изоляции': f'Русский ~ {stripping}',
                'Резка провода до ∅,': f'Русский ~ {wire_cutting}',
                'Резка стальной проволоки до ∅,': f'Русский ~ {steel_wire_cutting}',
                'Резка винтов': f'Русский ~ {screw_cutting}',

            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('universalnyy-instrument.csv', 'w', newline='', encoding='utf-8-sig') as f:
