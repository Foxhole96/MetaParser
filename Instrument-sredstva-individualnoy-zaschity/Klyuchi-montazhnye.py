...
url = 'https://001.com.ua/klyuchi-montazhnye'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            device_type = test_pars_capitalize(soup=soup, name='Тип ключа')
            nut_size = test_pars_capitalize(soup=soup, name='Размер гайки, мм')
            ...
            data.append({
                ...
                'Размер гайки': f'Русский ~ {nut_size}',
            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('klyuchi-montazhnye.csv', 'w', newline='', encoding='utf-8-sig') as f:
