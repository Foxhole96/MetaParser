...
url = 'https://001.com.ua/instrument-dlya-opressovki-nakonechnikov'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            crimped_product_type = test_pars_capitalize(soup=soup, name='Тип обжимаемых изделий')
            crimping = test_pars(soup=soup, name='Обжим, мм²')
            compression_profile = test_pars_capitalize(soup=soup, name='Профиль обжатия')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Инструмент для опрессовки наконечников',
                ...
                'Тип изделия': f'Русский ~ {device_type}',
                'Тип обжимаемых изделий': f'Русский ~ {crimped_product_type}',
                'Обжим, мм²': f'Русский ~ {crimping}',
                'Профиль обжатия': f'Русский ~ {compression_profile}',

            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('instrument-dlya-opressovki-nakonechnikov.csv', 'w', newline='', encoding='utf-8-sig') as f:
