...
url = 'https://001.com.ua/predohraniteli-plavkie-modulnye'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            try:
                size_td = soup.find('td', string='Типоразмер, мм').find_next('td')
                size_select = size_td.find('select',
                                                                 class_='form-control form-control-auto submit-on-change')
                if size_select:
                    size = size_select['title']
                else:
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Предохранители плавкие модульные',
                ...
                'Типоразмер': f'Русский ~ {size} ;; Украинский ~ {size}'
            })

            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('predohraniteli-plavkie-modulnye.csv', 'w', encoding='utf-8-sig') as f:
