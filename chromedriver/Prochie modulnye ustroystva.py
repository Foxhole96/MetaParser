...
url = 'https://001.com.ua/prochie-modulnye-ustroystva'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            try:
                device_type_td = soup.find('td', string='Тип устройства').find_next(
                    'td')
                device_type_select = device_type_td.find('select', class_='form-control form-control-auto submit-on-change')
                if device_type_select:
                    device_type = device_type_select['title'].capitalize()
                else:
            ...
            try:
                color_td = soup.find('td', string='Цвет').find_next(
                    'td')
                color_select = color_td.find('select', class_='form-control form-control-auto submit-on-change')
                if color_select:
                    color = color_select['title']
                else:
                    color = color_td.text.strip()
            except AttributeError:
                color = 'ERROR'
            ...
            try:
                device_type_ua_td = soup_ua.find('td', string='Тип пристрою').find_next(
                    'td')
                device_type_ua_select = device_type_ua_td.find('select',
                ...
                if device_type_ua_select:
                    ...
                    device_type_ua = device_type_ua_td.text.capitalize()
            except AttributeError:
                device_type_ua = 'ERROR'

            try:
                color_ua_td = soup.find('td', string='Цвет').find_next(
                    'td')
                color_ua_select = color_ua_td.find('select', class_='form-control form-control-auto submit-on-change')
                if color_ua_select:
                    color_ua = color_ua_select['title']
                else:
                    color_ua = color_ua_td.text.strip()
            except AttributeError:
                color_ua = 'ERROR'
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Дополнительные устройства модульной серии',
                ...
                'Тип устройства': f'Русский ~ {device_type} ;; Украинский ~ {device_type_ua}',
        ...
        with open('prochie-modulnye-ustroystva.csv', 'w', encoding='utf-8-sig') as f:
