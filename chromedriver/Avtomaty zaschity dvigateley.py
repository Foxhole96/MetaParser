...
url = 'https://001.com.ua/avtomaty-zaschity-dvigateley'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            try:
                arts_td = soup.find('td', string='Диапазон регулирования уставки теплового расцепителя, А').find_next(
                    'td')
                arts_select = arts_td.find('select', class_='form-control form-control-auto submit-on-change')
                if arts_select:
                    arts = arts_select['title']
                else:
                    arts = arts_td.text.strip()
            except AttributeError:
                arts = 'ERROR'
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Автоматы защиты двигателей',
                ...
                'Диапазон регулирования уставки теплового расцепителя': f'Русский ~ {arts} ;; Украинский ~ {arts}',

            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('avtomaty-zaschity-dvigateley.csv', 'w', encoding='utf-8-sig') as f:
            writer = csv.DictWriter(f, fieldnames=['URL',
                                                   ...
                                                   'Диапазон регулирования уставки теплового расцепителя',
                                                   ])
            writer.writeheader()
            for data in data:
