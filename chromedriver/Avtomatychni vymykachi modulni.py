...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            try:
                shutdown_characteristic_td = soup.find('td', string='Характеристика отключения').find_next('td')
                shutdown_characteristic_select = shutdown_characteristic_td.find('select',
                                                                                 class_='form-control form-control-auto submit-on-change')
                if shutdown_characteristic_select:
                    shutdown_characteristic = shutdown_characteristic_select['title']
                else:
                    shutdown_characteristic = shutdown_characteristic_td.text.strip()
            except AttributeError:
                shutdown_characteristic = 'ERROR'
            ...
            try:
                ...
                if maximum_breaking_capacity_select:
                    maximum_breaking_capacity = maximum_breaking_capacity_select['title']
                else:
            ...
            try:
                pole_width_td = soup.find('td', string='Ширина 1 полюса, мм').find_next('td')
                pole_width_select = pole_width_td.find('select',
                                                       class_='form-control form-control-auto submit-on-change')
                if pole_width_select:
                    pole_width = pole_width_select['title']
                else:
                    pole_width = pole_width_td.text.strip()
            except AttributeError:
                pole_width = 'ERROR'
            ...
            data.append({
                ...
                'Характеристика отключения': f'Русский ~ {shutdown_characteristic} ;; Украинский ~ {shutdown_characteristic}',
                'Максимальная отключающая способность, кА': f'Русский ~ {maximum_breaking_capacity} ;; Украинский ~ {maximum_breaking_capacity}',
                'Ширина 1 полюса, мм':  f'Русский ~ {pole_width} ;; Украинский ~ {pole_width}',
            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)


        with open('avtomaticheskie-vyklyuchateli-modulnye.csv', 'w', encoding='utf-8-sig') as f:
