...
url = 'https://001.com.ua/ogranichiteli-impulsnyh-perenapryazheniy-uzip'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            try:
                device_class_td = soup.find('td', string='Номинальный ток, А').find_next('td')
                device_class_select = device_class_td.find('select',
                                                                 class_='form-control form-control-auto submit-on-change')
                if device_class_select:
                    device_class = device_class_select['title']
                else:
                    device_class = device_class_td.text.strip()
            except AttributeError:
                device_class = 'ERROR'
            ...
            data.append({
                ...
                'Класс': f'Русский ~ {device_class} ;; Украинский ~ {device_class}',
        ...
        with open('ogranichiteli-impulsnyh-perenapryazheniy-uzip.csv', 'w', encoding='utf-8-sig') as f:
