...
url = 'https://001.com.ua/kontaktory-modulnye'
        ...
        for product_url in all_product_links:
            ...
            try:
                ...
                if count_of_poluses_select:
                    count_of_poluses = count_of_poluses_select['title']
                else:
            ...
            try:
                rvcc_td = soup.find('td', string='Номинальное напряжение катушки управления, В').find_next('td')
                rvcc_select = rvcc_td.find('select',
                                                                 class_='form-control form-control-auto submit-on-change')
                if rvcc_select:
                    rvcc = rvcc_select['title']
                else:
                    rvcc = rvcc_td.text.strip()
            except AttributeError:
                rvcc = 'ERROR'
            ...
            try:
                number_type_contacts_td = soup.find('td', string='Количество и вид контактов').find_next('td')
                number_type_contacts_select = number_type_contacts_td.find('select',
                                                                 class_='form-control form-control-auto submit-on-change')
                if number_type_contacts_select:
                    number_type_contacts = number_type_contacts_select['title']
                else:
                    number_type_contacts = number_type_contacts_td.text.strip()
            except AttributeError:
                number_type_contacts = 'ERROR'
            ...
            try:
                number_type_contacts_ua_td = soup_ua.find('td', string='Кількість та вид контактів').find_next('td')
                number_type_contacts_ua_select = number_type_contacts_ua_td.find('select',
                                                                 class_='form-control form-control-auto submit-on-change')
                if number_type_contacts_ua_select:
                    number_type_contacts_ua = number_type_contacts_ua_select['title']
                else:
                    number_type_contacts_ua = number_type_contacts_ua_td.text.strip()
            except AttributeError:
                number_type_contacts_ua = 'ERROR'
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Контакторы модульные',
                ...
                'Номинальное напряжение катушки управления': f'Русский ~ {rvcc} ;; Украинский ~ {rvcc}',
                'Количество и вид контактов': f'Русский ~ {number_type_contacts} ;; Украинский ~ {number_type_contacts_ua}'
            })

            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('kontaktory-modulnye.csv', 'w', encoding='utf-8-sig') as f:
