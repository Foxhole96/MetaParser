...
url = 'https://001.com.ua/provoda-shnury-mednye'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            cable_type = test_pars(soup=soup, name='Марка провода/шнура')
            wire_flexibility = test_pars_capitalize(soup=soup, name='Гибкость провода')
            cable_section = test_pars(soup=soup, name='Сечение жилы, мм²')
            num_of_cores = test_pars(soup=soup, name='Количество жил')
            color = test_pars_capitalize(soup=soup, name='Цвет оболочки')
            if color == 'По согласованию':
                color = ''

            if int(num_of_cores) > 1:
                type_of_cable = 'Многожильный'
            else:
                type_of_cable = 'Одножильный'



            print(num_of_cores, cable_section)
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Кабель электрический',
                ...
                'Тип кабеля': f'Русский ~ {cable_type}',
                'Гибкость провода': f'Русский ~ {wire_flexibility}',
                'Сечение кабеля': f'Русский ~ {cable_section}',
                'Кол-во проводников': f'Русский ~ {num_of_cores}',
                'Материал проводников': f'Русский ~ CU - медь',
                'Тип проводника': f'Русский ~ {type_of_cable}',
        ...
        with open('provoda-shnury-mednye.csv', 'w', newline='', encoding='utf-8-sig') as f:
