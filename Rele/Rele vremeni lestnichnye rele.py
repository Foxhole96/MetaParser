...
url = 'https://001.com.ua/rele-vremeni-lestnichnye-rele'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            dopolnitelno = test_pars_capitalize(soup=soup, name='Функции')

            trip_time = test_pars(soup=soup, name='Диапазон уставок времени срабатывания')

            num_of_contacts = test_pars(soup=soup, name='Количество контактов')

            num_of_settings = test_pars(soup=soup, name='Количество уставок')

            rated_voltage = test_pars(soup=soup, name='Номинальное напряжение, В')
            ...
            try:
                ...
                if montazh_select:
                    ...
                    if montazh == 'на DIN-рейку':
                        montazh = 'DIN-рейка'
                    elif montazh == 'в монтажную коробку':
                        montazh = 'В монтажную коробку'
                else:
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Реле времени, лестничные реле',
                ...
                'Диапазон уставок времени срабатывания': f'Русский ~ {trip_time}',
                'Количество контактов': f'Русский ~ {num_of_contacts}',
                'Количество уставок': f'Русский ~ {num_of_settings}',
                'Номинальное напряжение': f'Русский ~ {rated_voltage}',
        ...
        with open('rele-vremeni-lestnichnye-rele.csv', 'w', encoding='utf-8-sig') as f:
