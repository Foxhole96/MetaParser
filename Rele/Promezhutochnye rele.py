...
url = 'https://001.com.ua/promezhutochnye-rele'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            contact_groups = test_pars(soup=soup, name='Количество групп контактов')
            nominal_current = test_pars(soup=soup, name='Номинальный ток, А')
            coil_voltage = test_pars(soup=soup, name='Напряжение катушки управления, В')

            try:
                dopolnitelno_td = soup.find('td', string='Наличие индикации').find_next('td')
                ...
                if dopolnitelno_select:
                    ...
                    if dopolnitelno == 'есть':
                        dopolnitelno = 'Наличие индикации'
                    else:
                        dopolnitelno = ''
                else:
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Промежуточные реле',
                ...
                'Количество групп контактов': f'Русский ~ {contact_groups}',
                'Номинальный ток': f'Русский ~ {nominal_current}',
                'Напряжение катушки управления': f'Русский ~ {coil_voltage}',
        ...
        with open('promezhutochnye-rele.csv', 'w', encoding='utf-8-sig') as f:
