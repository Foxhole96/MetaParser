...
url = 'https://001.com.ua/rele-kontrolya-temperatury'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            l_temperature_limit = test_pars(soup=soup, name='Нижний предел уставки температуры, °C')
            h_temperature_limit = test_pars(soup=soup, name='Верхний предел уставки температуры, °C')
            num_of_channels = test_pars(soup=soup, name='Количество каналов')
            max_current = test_pars(soup=soup, name='Максимальный ток, А')

            try:
                dopolnitelno_td = soup.find('td', string='Датчик температуры в комплекте').find_next('td')
                ...
                if dopolnitelno_select:
                    ...
                    if dopolnitelno == 'есть':
                        dopolnitelno = 'Датчик температуры в комплекте'
                    else:
                        dopolnitelno = ''
                else:
            ...
            data.append({
                ...
                'Нижний предел уставки температуры': f'Русский ~ {l_temperature_limit}',
                'Верхний предел уставки температуры': f'Русский ~ {h_temperature_limit}',
                'Количество каналов': f'Русский ~ {num_of_channels}',
                'Максимальный ток': f'Русский ~ {max_current}',
                'Тип устройства': f'Русский ~ Реле контроля температуры',
        ...
        with open('rele-kontrolya-temperatury.csv', 'w', encoding='utf-8-sig') as f:
            writer = csv.DictWriter(f, fieldnames=['URL',
                                                   ...
                                                   'Нижний предел уставки температуры',
                                                   'Верхний предел уставки температуры',
                                                   'Количество каналов',
                                                   'Максимальный ток',
                                                   'Тип устройства',
                                                   'Способ монтажа',
