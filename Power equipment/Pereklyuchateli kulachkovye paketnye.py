...
url = 'https://001.com.ua/pereklyuchateli-kulachkovye-paketnye'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in all_product_links:
            ...
            performance = test_pars_capitalize(soup=soup, name='Исполнение')
            nominal_current = test_pars(soup=soup, name='Номинальный ток, А')
            provision_designations = test_pars(soup=soup, name='Обозначение положений')
            numb_of_poles = test_pars(soup=soup, name='Количество полюсов')
            degree_of_protection = test_pars(soup=soup, name='Степень защиты')
            possibility_of_sealing = test_pars(soup=soup, name='Возможность опломбирования')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Переключатели кулачковые (пакетные)',
                ...
                'Исполнение': f'Русский ~ {performance}',
                'Номинальный ток': f'Русский ~ {nominal_current}',
                'Обозначение положений': f'Русский ~ {provision_designations}',
                'Количество полюсов': f'Русский ~ {numb_of_poles}',
                'Степень защиты': f'Русский ~ {degree_of_protection}',
                'Возможность опломбирования': f'Русский ~ {possibility_of_sealing}',
        ...
        with open('pereklyuchateli-kulachkovye-paketnye.csv', 'w', encoding='utf-8-sig') as f:
