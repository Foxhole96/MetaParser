...
url = 'https://001.com.ua/yaschiki-s-ponizhayuschim-transformatorom'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            primary_voltage = test_pars(soup=soup, name='Напряжение первичной обмотки, В')
            secondary_voltage = test_pars(soup=soup, name='Напряжение вторичной обмотки, В')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Ящики с понижающим трансформатором',
                ...
                'Напряжение первичной обмотки': f'Русский ~ {primary_voltage}',
                'Напряжение вторичной обмотки': f'Русский ~ {secondary_voltage}',
            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('yaschiki-s-ponizhayuschim-transformatorom.csv', 'w', newline='', encoding='utf-8-sig') as f:
