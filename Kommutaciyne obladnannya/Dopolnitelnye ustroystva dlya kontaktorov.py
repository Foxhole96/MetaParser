...
url = 'https://001.com.ua/dopolnitelnye-ustroystva-dlya-kontaktorov'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            contactor_series = test_pars(soup=soup, name='Серия контакторов')
            reversing_device = test_pars(soup=soup, name='Устройство для реверса')
            delay_prefix = test_pars(soup=soup, name='Приставка выдержки времени')
            prefix_contact = test_pars(soup=soup, name='Приставка контактная')
            control_coil = test_pars(soup=soup, name='Катушка управления на напряжение')
            surge_arrester_for_coils = test_pars(soup=soup, name='Ограничитель перенапряжений для катушек')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Дополнительные устройства для контакторов',
                ...
                'Серия контакторов': f'Русский ~ {contactor_series}',
                'Устройство для реверса': f'Русский ~ {reversing_device}',
                'Приставка выдержки времени': f'Русский ~ {delay_prefix}',
                'Приставка контактная': f'Русский ~ {prefix_contact}',
                'Катушка управления на напряжение': f'Русский ~ {control_coil}',
                'Ограничитель перенапряжений для катушек': f'Русский ~ {surge_arrester_for_coils}',
        ...
        with open('dopolnitelnye-ustroystva-dlya-kontaktorov.csv', 'w', newline='', encoding='utf-8-sig') as f:
