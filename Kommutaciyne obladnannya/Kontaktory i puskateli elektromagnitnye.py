...
url = 'https://001.com.ua/kontaktory-i-puskateli-elektromagnitnye'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            nominal_current = test_pars(soup=soup, name='Номинальный рабочий ток, А')
            nominal_coil_voltage = test_pars(soup=soup, name='Номинальное напряжение катушки управления, В')
            num_and_type_of_contacts = test_pars(soup=soup, name='Количество и вид дополнительных контактов')
            device_type = test_pars_capitalize(soup=soup, name='Тип')
            shell_presence = test_pars(soup=soup, name='Наличие оболочки')
            thermal_rele_presence = test_pars_capitalize(soup=soup, name='Наличие теплового реле')
            thermal_relay_setting_range = test_pars(soup=soup, name='Диапазон уставок теплового реле, А')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Контакторы и пускатели электромагнитные',
                ...
                'Номинальный рабочий ток': f'Русский ~ {nominal_current}',
                'Номинальное напряжение катушки управления': f'Русский ~ {nominal_coil_voltage}',
                'Количество и вид контактов': f'Русский ~ {num_and_type_of_contacts}',
                'Тип': f'Русский ~ {device_type}',
                'Наличие оболочки': f'Русский ~ {shell_presence}',
                'Наличие теплового реле': f'Русский ~ {thermal_rele_presence}',
                'Диапазон уставок теплового реле': f'Русский ~ {thermal_relay_setting_range}',

            })
            driver.find_element(By.XPATH, '/html/body/header/nav/div/div[2]/ul[2]/li[1]/a/span[1]').click()
            time.sleep(2)

        with open('kontaktory-i-puskateli-elektromagnitnye.csv', 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.DictWriter(f, fieldnames=['URL',
                                                   ...
                                                   'Диапазон уставок теплового реле',
