...
url = 'https://001.com.ua/silovye-nakonechniki-i-gilzy'
...
def get_cat_html_ru():
    ...
    try:
        ...
        for product_url in tqdm(all_product_links, desc="Парсинг", unit="посилання"):
            ...
            material = test_pars_capitalize(soup=soup, name='Материал наконечника')
            cable_section = test_pars(soup=soup, name='Сечение кабеля, мм²')
            termination_method = test_pars(soup=soup, name='Метод оконцевания')
            hole_diametr = test_pars(soup=soup, name='Диаметр отверстия под болт, мм')
            ...
            data.append({
                ...
                'images': images_str,
                'item_name': title,
                'category_name': 'Силовые наконечники и гильзы',
                ...
                'Метод оконцевания': f'Русский ~ {termination_method}',
                'Диаметр отверстия под болт': f'Русский ~ {hole_diametr}',
        ...
        with open('silovye-nakonechniki-i-gilzy.csv', 'w', newline='', encoding='utf-8-sig') as f:
